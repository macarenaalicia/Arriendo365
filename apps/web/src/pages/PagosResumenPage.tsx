...
function calcularEstadoCelda(
  ...
  if (delMes.length === 0) {
    ...
    if (mesKey === mesActualKey && diaPago !== null) {
      ...
      // Por ley hay plazo hasta el día 5 para pagar el arriendo, aunque el
      // día pactado sea antes — no se marca "atrasado" antes de esa fecha.
      const ultimoDiaDelMes = new Date(hoy.getFullYear(), hoy.getMonth() + 1, 0).getDate();
      const diaPagoEfectivo = Math.min(Math.max(diaPago, 5), ultimoDiaDelMes);
